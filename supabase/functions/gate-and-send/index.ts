...
// =============================================================
// AS MARCAS — uma função, dois clientes
//
// 🔑 Esta função é COMPARTILHADA. O respondente carrega `evento_id`, o evento
//    carrega `cliente`, e é o cliente que decide cor, fonte, textos, assunto e
//    nome do anexo. Antes disso, o PDF saía IBMEC para todo mundo: quem
//    respondesse a degustação da Boomit via relatório Boomit na tela, baixava
//    PDF Boomit no botão — e recebia um PDF IBMEC no e-mail.
//
// 🔒 IBMEC está com os valores EXATOS que já estavam no ar, um a um. Para eles
//    esta mudança tem que ser invisível: mesmo PDF, mesmo assunto, mesmo nome
//    de arquivo. Qualquer diferença aqui é bug, não melhoria.
//
//    Boomit vem de 3-produto/regras/design/boomit-design-tokens.json (guia
//    BerrielBrands 2023), os mesmos tokens do formulário e do relatório na
//    tela — não amostrados, copiados da fonte de verdade.
// =============================================================
interface Marca {
  nome: string;
  remetente: string;
  ...
  fonteHref: string;
  fonteFamily: string;
  primaria: string; acento: string; texto: string; textoSuave: string;
  muted: string; linha: string; bloco: string; metaBloco: string; destaque: string;
  ok: string; erro: string; atencao: string;
  reguaGradiente: string;
  radarEmpresa: string; radarPessoa: string;
  pontoAtual: string;
}
...
const MARCAS: Record<string, Marca> = {
  IBMEC: {
    nome: 'IBMEC',
    remetente: 'Diagnostico IBMEC',
    arquivo: 'diagnostico-ibmec',
    rodapeEmail: 'IBMEC · Diagnostico Estrategico · Junho 2026',
    fonteHref: 'https://fonts.googleapis.com/css2?family=Krub:wght@400;500;600;700&display=swap',
    fonteFamily: `'Krub', Tahoma, sans-serif`,
    primaria: '#002555', acento: '#F5AC00', texto: '#2A3754', textoSuave: '#4A5670',
    muted: '#6C7A92', linha: '#E2E6EF', bloco: '#F4F5F8', metaBloco: '#EEF1F8', destaque: '#FFF4D6',
    ok: '#10A981', erro: '#D14343', atencao: '#E89B1F',
    reguaGradiente: 'linear-gradient(to right, #FFF4D6, #F5AC00, #002555)',
    radarEmpresa: '#F5AC00', radarPessoa: '#002555',
    pontoAtual: 'preto',
  },
  Boomit: {
    nome: 'Boomit',
    remetente: 'Diagnostico Boomit',
    arquivo: 'diagnostico-boomit',
    rodapeEmail: 'Boomit · Diagnostico Estrategico de Lideranca',
    // PP Mori é a fonte da marca, mas não é webfont pública — o produto no ar
    // resolve com Inter, e aqui seguimos a mesma escolha.
    fonteHref: 'https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&display=swap',
    fonteFamily: `'Inter', system-ui, sans-serif`,
    primaria: '#545E54',   // verde assinatura — a única cor própria da marca
    acento: '#6F5525',     // gold um passo abaixo: em 9pt caixa alta o token
                           // puro (#9A7B3C) não cruza 4.5:1 no papel branco
    texto: '#1B1B1C', textoSuave: '#3F4340',
    muted: '#6E746A', linha: '#DAD4CB', bloco: '#EDE9E4', metaBloco: '#E5E0D8', destaque: '#F0E9DC',
    ok: '#5C7A5C', erro: '#A0543E', atencao: '#9D9482',
    // a régua sobe pela hierarquia da marca: operacional → tático → estratégico
    reguaGradiente: 'linear-gradient(to right, #E5E0D8, #A7A091, #545E54, #1B1B1C)',
    radarEmpresa: '#9A7B3C', radarPessoa: '#545E54',
    pontoAtual: 'verde',
  },
};
...
function gerarRadarSVG(scores: Record<string, any>, m: Marca): string {
  ...
  for (const v of [1, 2, 3, 4, 5]) {
    ...
    grid += `<polygon points="${pts}" fill="none" stroke="${m.linha}" stroke-width="1"/>`;
  }
  let axes = '';
  for (let i = 0; i < n; i++) {
    const x = cx + rMax * Math.cos(angles[i]);
    const y = cy + rMax * Math.sin(angles[i]);
    axes += `<line x1="${cx}" y1="${cy}" x2="${x.toFixed(1)}" y2="${y.toFixed(1)}" stroke="${m.linha}" stroke-width="1"/>`;
  }
  ...
  const polys = poly((d) => scores[d]?.empresa || 0, m.radarEmpresa, 0.4)
              + poly((d) => scores[d]?.pessoa  || 0, m.radarPessoa, 0.5);
  ...
  for (let i = 0; i < n; i++) {
    ...
    txt += `<text x="${x.toFixed(0)}" y="${y.toFixed(0)}" text-anchor="${anchor}" font-size="13" font-weight="700" fill="${m.primaria}">${labels[i]}</text>`;
  }
...
function renderPdfHtml(data: { respondente: any; relatorio: any; payload: GatePayload; m: Marca }): string {
  ...
  if (risco >= 60) { riscoNivel = 'Alto'; riscoCor = m.erro; }
  else if (risco >= 40) { riscoNivel = 'Moderado'; riscoCor = m.acento; }
  ...
  const compRows = competencias.slice(0, 5).map((c, i) => {
    ...
    return `<div class="comp-row"><div><span style="font-size:14pt;font-weight:800;color:${m.acento};">#${i + 1}</span> <span class="comp-name">${c.nome}</span></div><div class="comp-bar-track"><div class="comp-bar-fill" style="width:${pctAtual}%;"></div><div class="comp-bar-exp" style="width:80%;"></div></div><div class="comp-gap ${gapCls}">${gapSign}${c.gap.toFixed(1)}</div></div>`;
  }).join('');

  const compsHtml = componentes.map(c => {
    const tag = classificaTag(c.valor);
    return `<div class="ind-row"><div><div style="font-weight:600;font-size:10pt;">${c.nome}</div><div style="font-size:8.5pt;color:${m.muted};">${c.desc}</div></div><div class="ind-bar-track"><div class="ind-bar-fill" style="width:${c.valor}%;"></div></div><div class="ind-value">${c.valor.toFixed(0)}<span class="tag ${tag.cls}">${tag.label}</span></div></div>`;
  }).join('');

  const dimDestaque = competencias[0]?.nome || 'multiplas dimensoes';

  const css = `* { box-sizing: border-box; margin: 0; padding: 0; } @page { size: A4; margin: 16mm 14mm; } body { font-family: ${m.fonteFamily}; color: ${m.texto}; font-size: 11pt; line-height: 1.5; } h1 { font-size: 28pt; font-weight: 700; color: ${m.primaria}; line-height: 1.05; } h2 { font-size: 14pt; font-weight: 600; color: ${m.primaria}; margin-bottom: 4pt; padding-bottom: 5pt; border-bottom: 2pt solid ${m.acento}; } h3 { font-size: 10pt; font-weight: 700; color: ${m.primaria}; text-transform: uppercase; letter-spacing: 1pt; margin-top: 14pt; } p { margin-bottom: 8pt; } .eyebrow { font-size: 9pt; color: ${m.acento}; text-transform: uppercase; letter-spacing: 3pt; font-weight: 700; } .card { padding: 14pt; border: 1pt solid ${m.linha}; margin: 10pt 0; page-break-inside: avoid; } .card-sub { font-size: 9.5pt; color: ${m.muted}; margin-bottom: 8pt; font-style: italic; } .meta-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8pt; margin-top: 14pt; } .meta-item { background: ${m.metaBloco}; padding: 8pt 10pt; } .meta-label { font-size: 8pt; color: ${m.muted}; text-transform: uppercase; letter-spacing: 1.5pt; } .meta-value { font-size: 10pt; font-weight: 700; color: ${m.primaria}; margin-top: 2pt; } .header { padding-bottom: 14pt; border-bottom: 2pt solid ${m.primaria}; margin-bottom: 18pt; } .conceito-block { background: ${m.bloco}; padding: 12pt 14pt; border-left: 3pt solid ${m.acento}; margin: 8pt 0 12pt; font-size: 9.5pt; line-height: 1.55; } .conceito-block strong { color: ${m.primaria}; } .conceito-mini { background: white; padding: 8pt 10pt; margin-top: 8pt; font-size: 8.5pt; color: ${m.muted}; line-height: 1.5; border-left: 2pt solid ${m.linha}; } .subsidios-block { background: ${m.destaque}; padding: 11pt 14pt; margin: 12pt 0 0; border-left: 3pt solid ${m.acento}; font-size: 9pt; line-height: 1.55; color: ${m.textoSuave}; } .subsidios-block strong { display: block; color: ${m.primaria}; text-transform: uppercase; letter-spacing: 1pt; font-size: 8pt; margin-bottom: 4pt; } .grade-hero { display: grid; grid-template-columns: auto 1fr; gap: 22pt; align-items: center; margin: 12pt 0; } .grade-circle { width: 90pt; height: 90pt; border-radius: 50%; background: ${m.primaria}; color: white; display: flex; align-items: center; justify-content: center; font-size: 44pt; font-weight: 800; } .regua-zones { display: flex; justify-content: space-between; padding: 4pt 0; font-size: 9pt; color: ${m.muted}; font-weight: 600; text-transform: uppercase; letter-spacing: 1pt; } .regua-track { background: ${m.reguaGradiente}; height: 26pt; position: relative; border: 1pt solid ${m.linha}; } .regua-marker { position: absolute; top: -8pt; transform: translateX(-50%); } .regua-dot { width: 14pt; height: 14pt; background: ${m.primaria}; border-radius: 50%; border: 2pt solid white; margin: 0 auto; } .regua-label { font-size: 8pt; font-weight: 700; margin-top: 3pt; text-align: center; white-space: nowrap; } .comp-row { display: grid; grid-template-columns: 1fr 2fr auto; gap: 8pt; padding: 7pt 0; border-bottom: 1pt solid ${m.linha}; align-items: center; } .comp-name { font-weight: 600; font-size: 10pt; } .comp-bar-track { height: 10pt; background: ${m.linha}; position: relative; } .comp-bar-fill { position: absolute; height: 100%; background: ${m.primaria}; } .comp-bar-exp { position: absolute; height: 100%; border-right: 2pt solid ${m.acento}; } .comp-gap { font-size: 10pt; font-weight: 700; } .comp-gap.neg { color: ${m.erro}; } .ind-row { display: grid; grid-template-columns: 1fr 2fr auto; gap: 8pt; padding: 8pt 0; border-bottom: 1pt solid ${m.linha}; align-items: center; } .ind-bar-track { height: 9pt; background: ${m.linha}; position: relative; } .ind-bar-fill { position: absolute; height: 100%; background: ${m.primaria}; } .ind-value { font-weight: 700; font-size: 11pt; color: ${m.primaria}; min-width: 50pt; text-align: right; } .tag { font-size: 8pt; padding: 2pt 6pt; text-transform: uppercase; letter-spacing: 0.5pt; font-weight: 700; margin-left: 4pt; } .tag-forte { background: ${m.ok}; color: white; } .tag-saudavel { background: ${m.acento}; color: ${m.primaria}; } .tag-atencao { background: ${m.atencao}; color: white; } .tag-critico { background: ${m.erro}; color: white; } .risco-hero { display: grid; grid-template-columns: auto 1fr; gap: 18pt; align-items: center; } .risco-circle { width: 80pt; height: 80pt; border-radius: 50%; border: 5pt solid ${riscoCor}; display: flex; align-items: center; justify-content: center; font-size: 22pt; font-weight: 800; color: ${m.primaria}; } .cdl-box { background: ${m.bloco}; padding: 14pt; border-left: 4pt solid ${m.acento}; margin: 10pt 0; } .cdl-value { font-size: 20pt; font-weight: 800; color: ${m.primaria}; margin: 6pt 0; } .cdl-decomp { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8pt; margin-top: 10pt; } .cdl-decomp-item { background: white; padding: 8pt; border: 1pt solid ${m.linha}; } .cdl-decomp-label { font-size: 8pt; text-transform: uppercase; letter-spacing: 1.5pt; color: ${m.muted}; font-weight: 600; } .cdl-decomp-value { font-size: 10pt; font-weight: 700; color: ${m.primaria}; margin-top: 3pt; } .plano-item { display: grid; grid-template-columns: 30pt 1fr; gap: 10pt; padding: 10pt 0; border-bottom: 1pt solid ${m.linha}; align-items: start; } .plano-num { width: 26pt; height: 26pt; border-radius: 50%; background: ${m.primaria}; color: white; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 11pt; } .plano-content h4 { font-size: 11pt; font-weight: 700; color: ${m.primaria}; margin-bottom: 3pt; } .plano-content ul { margin-left: 14pt; font-size: 10pt; } .crono-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8pt; margin-top: 10pt; } .crono-etapa { background: ${m.bloco}; padding: 10pt; border-top: 3pt solid ${m.acento}; } .crono-num { font-size: 9pt; text-transform: uppercase; letter-spacing: 1.5pt; color: ${m.muted}; font-weight: 700; } .crono-titulo { font-size: 11pt; font-weight: 700; color: ${m.primaria}; margin: 3pt 0 6pt; } .crono-bullets { font-size: 9pt; color: ${m.muted}; } .crono-bullets li { margin: 2pt 0 2pt 12pt; } .gloss-item { padding: 6pt 0; border-bottom: 1pt dashed ${m.linha}; font-size: 10pt; } .gloss-term { font-weight: 700; color: ${m.primaria}; } .gloss-def { color: ${m.muted}; } .sources { background: ${m.bloco}; padding: 10pt 14pt; font-size: 9pt; color: ${m.muted}; line-height: 1.6; } .footer-doc { text-align: center; font-size: 8pt; color: ${m.muted}; padding: 10pt 0; margin-top: 14pt; border-top: 1pt solid ${m.linha}; } .radar-wrap { text-align: center; margin: 14pt 0; } .radar-legend { display: flex; justify-content: center; gap: 18pt; margin-top: 8pt; font-size: 9pt; color: ${m.muted}; } .legend-sw { display: inline-block; width: 10pt; height: 10pt; vertical-align: middle; margin-right: 4pt; } .page-break { page-break-before: always; }`;

  const header = `<div class="header"><div style="display:flex;justify-content:space-between;align-items:end;"><div><div class="eyebrow">Relatorio Estrategico</div><h1 style="margin-top:6pt;">${r.empresa || r.nome || 'Diagnostico'}</h1></div><div style="text-align:right;"><div class="eyebrow">${m.nome}</div><div style="font-size:9pt;color:${m.muted};margin-top:3pt;">${dataPt}</div></div></div><div class="meta-grid"><div class="meta-item"><div class="meta-label">Respondente</div><div class="meta-value">${r.nome || '—'}</div></div><div class="meta-item"><div class="meta-label">Nivel</div><div class="meta-value">${NIVEL_LABEL[r.persona] || '—'}</div></div><div class="meta-item"><div class="meta-label">Porte / Setor</div><div class="meta-value">${PORTE_LABEL[r.tamanho] || '—'} · ${SETOR_LABEL[r.segmento] || '—'}</div></div></div></div>`;

  const sec1 = `<div class="card"><h2>Indicador de Maturidade</h2><p class="card-sub">Estagio de maturidade de lideranca da organizacao e do respondente, consolidado nas 5 dimensoes avaliadas.</p><div class="conceito-block">A maturidade de uma organizacao nao e uma virtude moral — e uma <strong>descricao estrutural</strong>. Inspirada em frameworks como o CMMI (Software Engineering Institute) e o Organizational Health Index (McKinsey), nossa escala D→AAA descreve em que estagio sua empresa joga hoje e calibra a regua de cobranca realista. Empresa em nivel B com cobranca de nivel AAA gera burnout sistemico; empresa AAA com governanca de B perde competitividade. <strong>A leitura honesta do estagio e o ponto de partida de qualquer plano de evolucao viavel.</strong><div class="conceito-mini">Como ler: a letra reflete o estagio dominante; o score 0-100 detalha a posicao dentro do estagio. Mudar de letra e um movimento de 12-18 meses; subir score dentro da mesma letra e alcancavel em 90 dias com intervencao estruturada.</div></div><div class="grade-hero"><div style="text-align:center;"><div class="grade-circle">${rel.maturidade_letra || 'B'}</div><div style="margin-top:6pt;font-size:9pt;color:${m.muted};">Score: ${(rel.maturidade_score || 0).toFixed(0)}/100</div></div><div><h3 style="margin-top:0;">Diagnostico</h3><p style="font-size:10pt;">${diagMaturidade(rel.maturidade_letra || 'B')}</p></div></div><div class="subsidios-block"><strong>Subsidio para conversa</strong>Use o diagnostico textual acima para validar com seu time se a leitura ressoa. Divergencia forte entre o que o instrumento mostra e o que a equipe sente e sinal de pontos cegos que merecem investigacao dedicada nos proximos 30 dias.</div></div>`;

  const sec2 = `<div class="card page-break"><h2>Regua de Posicionamento</h2><p class="card-sub">Onde sua capacidade de lideranca esta sendo gasta hoje versus onde deveria estar, considerando seu nivel hierarquico.</p><div class="conceito-block">Toda funcao carrega uma <strong>assinatura esperada</strong> de distribuicao de tempo entre tres planos: Operacional (execucao do dia), Tatico (coordenacao trimestral) e Estrategico (visao de 6+ meses). Um Diretor com 60% do tempo no Operacional nao esta sendo dedicado — esta sendo <em>arrastado</em> pelo curto prazo, e a empresa paga o custo dessa centralizacao em gargalo de decisao e atraso estrutural. A Regua mostra onde sua capacidade esta concentrada hoje versus onde, dado seu nivel, ela deveria estar.<div class="conceito-mini">Como ler: ponto ${m.pontoAtual} = posicao atual; ponto dourado = posicao esperada para seu nivel hierarquico. A distancia entre os dois e o <em>gap de posicionamento</em> — quanto maior, maior a indicacao de trabalho estrutural a fazer (delegacao, processos, sucessao).</div></div><div style="background:${m.bloco};padding:18pt 14pt;margin:14pt 0;"><div class="regua-zones"><span>Operacional</span><span>Tatico</span><span>Estrategico</span></div><div class="regua-track"><div class="regua-marker" style="left:${stratPct}%;"><div class="regua-dot"></div><div class="regua-label">Atual</div></div><div class="regua-marker" style="left:${esperado}%;top:-8pt;"><div class="regua-dot" style="background:${m.acento};"></div><div class="regua-label" style="color:${m.acento};">Esperado</div></div></div></div><p style="font-size:10pt;">${leituraRegua(stratPct, esperado, r.persona)}</p><div class="subsidios-block"><strong>Subsidio para conversa</strong>Gap acima de 15% costuma sinalizar tres causas frequentes: (i) ausencia de camada de execucao madura abaixo de voce, (ii) falta de confianca nos diretos para delegar, ou (iii) processos nao documentados que forcam voce a operar no detalhe. Cada uma demanda uma intervencao distinta.</div></div>`;
  ...
  const footer = `<div class="footer-doc">Documento confidencial · Diagnostico Estrategico ${m.nome} · Gerado em ${dataPt}</div>`;

  return `<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Diagnostico ${m.nome}</title><link href="${m.fonteHref}" rel="stylesheet"><style>${css}</style></head><body>${header}${sec1}${sec2}${sec3}${sec4}${sec5}${sec6}${sec7}${sec8}${sec9}${sec10}${sec11}${sources}${footer}</body></html>`;
}
...
async function sendEmailBrevo(opts: { to: string; toName: string; subject: string; html: string; pdfBytes: Uint8Array | null; pdfName: string; m: Marca }): Promise<void> {
  const apiKey = Deno.env.get('BREVO_API_KEY');
  if (!apiKey) throw new Error('BREVO_API_KEY nao configurado');
  const fromName = opts.m.remetente;
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // 🔑 A MARCA VEM DO EVENTO. Uma funcao so atende os dois clientes: o
    //    respondente carrega evento_id, o evento carrega `cliente`, e e o
    //    cliente que decide cor, fonte, textos, assunto e nome do anexo.
    //    Cliente desconhecido cai no padrao — melhor entregar com a marca
    //    errada do que nao entregar.
    const { data: evento } = await supabase
      .from('eventos').select('cliente').eq('id', respondente.evento_id).single();
    const m: Marca = MARCAS[evento?.cliente as keyof typeof MARCAS] ?? MARCAS.IBMEC;
    ...
    try {
      const html = renderPdfHtml({ respondente, relatorio, payload, m });
    ...
    try {
      ...
      await sendEmailBrevo({
        ...
        html: bodyHtml,
        pdfBytes,
        pdfName: `${m.arquivo}-${safeName}.pdf`,
      });
      emailEnviado = true;
